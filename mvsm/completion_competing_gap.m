...
cd files;
count=1;
for b = 2:2%length(bws)
    bw=bws(b);
    for bf = 2:2%4%buffs_dim(2) bdp,2bdp,4bdp are only available now
        buff=buffs(b,bf);
            ...
            for l = 1:length(lrs)
                ...
                for sc = 1:length(schemes)
                    ...
                    for e = 1:length(exp_sizes)
                        exp=exp_sizes(e);
                        for spc_idx = 1:length(spacings) %only 0.1 at idx 2
...
%Works after running the above extraction code on one buffer and all the spacings
%bar plot(spacings) for completion time of a Single CCA(cubic) for all CCA combinations for all bw and all buff sizes(BDP,2BDP,4BDP)
figure
lgd_str={};
eval_3cca_3buf=zeros(6,6)
cca_eval=ccas(2);
gap=spacings(1);
for bw_idx = 2:2%length(bws)
    bw=bws(bw_idx);
        ...
        for cca1_idx =1:length(ccas)
            ...
            for spc_idx=1:6
                ...
                if strcmp(cca_one,cca_eval)
                    eval_3cca_3buf(spc_idx, cca1_idx)=dval(1,:);
                else
                   eval_3cca_3buf(spc_idx,cca1_idx)=dval(2,:); 
                end
            end
        end
     end
...
xlabel('Inter-flow intervals(sec)','FontSize',24);
%ylabel(strcat('Completion time of small ',cca_eval,'flow'),'FontSize',18);
ylabel(strcat('Completion time of compet. ',' Cubic',' (sec)'),'FontSize',22);
alt_spacings=zeros(1,length(spacings));
for i=1:length(spacings)
    temp=char(spacings(i));
    alt_spacings(i)=str2num(temp(1:end-3));
end
set(gca,'XTick',1:6,'XTicklabel',alt_spacings,'FontSize',22);
%xlim([0 20]);%max 25 for 5mbit
%ylim([0 20]);%max 100 for 5mbit
lgd_arr={'vs bbr', 'vs cubic', 'vs newreno'};
lgd_str=lgd_arr;
lgd=legend(lgd_str);
lgd.FontSize=24;
title(strcat(cca_eval,'-',bw,'-','BDP'),'FontSize',24);
...
%one line plot for each bw and with the x-axis of spacings

figure
lgd_str={};
cca_eval=ccas(2);
gap='0.1gap';
buf_idx = 2;%:4%buffs_dim(2)
    ...
    for bw_idx = 2:4 % only for 10,30 and 40 mbits
        ...
        for spc_idx = 1:6%buffs_dim(2)
            ...
            if strcmp(cca_one,cca_eval)
                ...
                eval_allgap_1bw(1,spc_idx)=dval(2,:); %for all iterations buffer is always the same while bw changes
            end              
        end
...
xlabel('Inter-flow Spacings','FontSize',14);
%ylabel(strcat('Completion time of small ',cca_eval,'flow'),'FontSize',14);
set(gca,'XTick',1:6,'XTicklabel',spacings,'FontSize',14);
%xlim([0 20]);%max 25 for 5mbit
%ylim([0 20]);%max 100 for 5mbit
%lgd_str=buff_labels(2:4);
lgd=legend(lgd_str);
lgd.FontSize=12;
title(strcat(cca_eval,'-',lr,'-',sch,'-',exp,'-',buff_labels(buf_idx)),'FontSize',12);
