    ...
    for bdp_idx=4:4%buf_dim(2)
        ...
        for cca1_idx=1:3%length(ccas)
            ...
            for cca2_idx = 2:2%length(ccas)
                ...
                file_regex=char(strcat(dirct,cca_one,'_',first_data,'_',rtt,'_',cca_two,'_',second_data,'_',rtt,'_',bw,'_',buf,'_',lr,'_',sch,'_',space,'_','*','_',plotted_flow,'_',file_end1));
                files_list = dir(file_regex);
                all_rtts=[];
                for i=1:length(files_list)
                    rtts_per_iter=importdata(strcat(dirct,'/',files_list(i).name));
...
ppty=char(file_end1);
ppty=ppty(1:end-4);
ttle= char(strcat(ppty,'-cdf-of-',selected_cca,'-',sch,'-',space,'-',bw,'-',buf,'(4BDP)','-',first_data,'-',second_data))
title(ttle)
